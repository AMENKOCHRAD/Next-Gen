...
public class TraitementController {
    ...
    @FXML
    private ComboBox<Integer> reclamationComboBox;
    ...
    public void initialize() {
        List<Reclamation> reclamations = reclamationService.getAllData();
        for (Reclamation rec : reclamations) {
            reclamationComboBox.getItems().add(rec.getId());
        }
        ...
        reclamationComboBox.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue != null) {
                handleReclamationSelection();
            }
        });
    }
    ...
    @FXML
    private void handleReclamationSelection() {
        Integer selectedId = reclamationComboBox.getValue();
        if (selectedId != null) {
            Reclamation reclamation = reclamationService.getReclamationById(selectedId);
            if (reclamation != null) {
                ...
                if (reclamation.getPiecesJointes() != null && !reclamation.getPiecesJointes().isEmpty()) {
                    String imagePath = reclamation.getPiecesJointes().get(0);
    ...
    private void handleAjouterTraitement() {
        try {
            if (reclamationComboBox.getValue() == null) {
                showAlert("Erreur", "Veuillez sélectionner une réclamation.", Alert.AlertType.ERROR);
                return;
            }
            int reclamationId = reclamationComboBox.getValue();
            ...
            if (commentaire.length() < 10) {
                showAlert("Erreur", "Le commentaire doit contenir au moins 10 caractères.", Alert.AlertType.ERROR);
                return;
            }
            if (commentaire.length() > 500) {
                showAlert("Erreur", "Le commentaire ne doit pas dépasser 500 caractères.", Alert.AlertType.ERROR);
                return;
            }
            if (!commentaire.matches("^[a-zA-Z0-9.,!?\s]+$")) {
                showAlert("Erreur", "Le commentaire contient des caractères invalides.", Alert.AlertType.ERROR);
                return;
            }
            if (priorite == null) {
                showAlert("Erreur", "Veuillez sélectionner une priorité.", Alert.AlertType.ERROR);
                return;
            }
            if (statut.equals("RESOLU") && commentaire.length() < 20) {
                showAlert("Erreur", "Un statut 'RESOLU' nécessite un commentaire plus détaillé.", Alert.AlertType.ERROR);
                return;
            }
            ...
            try {
                ...
                // Fermer la fenêtre actuelle (si elle doit être fermée après l'ajout)
