...
public class DetailController {
    ...
    private FilteredList<Reclamation> filteredReclamations = new FilteredList<>(reclamations);
    ...
    public void initialize() {
        ...
        // Lier la TableView à la liste filtrée
    ...
    private void filterReclamations(String searchText) {
        filteredReclamations.setPredicate(reclamation -> {
            if (searchText == null || searchText.isEmpty()) {
                return true; // Afficher toutes les réclamations si la recherche est vide
            }

            // Filtrer par catégorie
            String lowerCaseFilter = searchText.toLowerCase();
            if (reclamation.getCategorie().toString().toLowerCase().contains(lowerCaseFilter)) {
                return true;
            }

            return false; // Ne pas afficher la réclamation si elle ne correspond pas
        });
    }
    ...
    private void handleSearch() {
        ...
        filterReclamations(searchField.getText());
    }
    ...
    @FXML
    private void handleTraiterReclamation() {
        Reclamation selectedReclamation = reclamationsTable.getSelectionModel().getSelectedItem();
        if (selectedReclamation != null) {
            try {
                // Charger la page Traitement.fxml
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/TraitementReclamation.fxml"));
                Parent root = loader.load();

                // Passer la réclamation sélectionnée au contrôleur de la page Traitement
                TraitementController traitementController = loader.getController();
                traitementController.setReclamation(selectedReclamation);

                // Afficher la page Traitement
                Stage stage = new Stage();
                stage.setScene(new Scene(root));
                stage.setTitle("Ajouter un Traitement");
                stage.show();
            } catch (IOException e) {
                System.err.println("Erreur lors du chargement de la page Traitement : " + e.getMessage());
                e.printStackTrace();
            }
        } else {
            Alert alert = new Alert(Alert.AlertType.WARNING);
